...
export default function AdminServicesPage() {
  ...
  const [createService] = useCreateAdminServiceMutation();
  const [updateService] = useUpdateAdminServiceMutation();
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this support service?')) {
      try {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      category: editingItem?.category || 'Integrations',
      icon: editingItem?.icon || 'Wrench',
      rating: editingItem?.rating || 5,
      reviews: editingItem?.reviews || 0,
      author: editingItem?.author || 'NovaKit Squad',
      delivery_days: editingItem?.delivery_days || 3,
      revision_limit: editingItem?.revision_limit || 3,
      highlightsStr: editingItem?.highlights?.join(', ') || 'Direct WhatsApp routing configurations, Twilio credentials mapping, Unlimited webhook receivers',
      bundlesStr: editingItem?.bundles ? JSON.stringify(editingItem.bundles, null, 2) : `{\n  "App.js": "export default function App() {\\n  return <div style={{padding: 24, background: '#09090b', color: '#fff', minHeight: '100vh'}}>Service preview</div>;\\n}"\n}`,
    },
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (editingItem) {
          await updateService({ id: editingItem._id, ...payload }).unwrap();
        ...
        console.error(err);
        toast.error('Failed to submit service form. Check JSON payload syntax.');
      }
    },
  });
  ...
  return (
    <div className="space-y-6 relative min-h-screen pb-16">
      <div className="flex items-center justify-between border-b border-neutral-200 dark:border-neutral-850 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-white">Manage Services</h1>
          <p className="mt-1.5 text-sm text-neutral-500">Configure engineering and custom setup support services.</p>
        </div>
        <button
          onClick={handleCreate}
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-655 hover:bg-indigo-700 text-white rounded-full text-xs font-semibold shadow-lg shadow-indigo-650/15"
        >
      ...
      {isLoading ? (
        <div className="p-10 text-center text-xs text-neutral-500">Fetching services collections...</div>
      ) : items.length === 0 ? (
        <div className="p-10 text-center text-xs text-neutral-500">No services seeded yet. Add one to begin.</div>
      ) : (
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-bold uppercase tracking-wider bg-indigo-50 dark:bg-indigo-950/40 text-indigo-655 dark:text-indigo-400 px-2.5 py-0.5 rounded-full">
                    {item.category}
                  </span>
                ...
                <div className="mt-4 flex gap-4 text-[10px] text-neutral-450 font-bold tracking-wider">
                  <span>DAYS: {item.delivery_days}</span>
                  <span>REVISIONS: {item.revision_limit}</span>
                </div>
              </div>
              ...
              <div className="mt-5 flex gap-2 border-t border-neutral-100 dark:border-neutral-850 pt-4">
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center bg-rose-50 dark:bg-rose-950/20 text-rose-605 dark:text-rose-450 hover:bg-rose-100 rounded-lg transition"
                >
            ...
            <div>
              <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-900 pb-4 mb-6">
                <div>
                  ...
                  <span className="text-[10px] text-neutral-400">ENGINEERING SUPPORT ASSET</span>
                </div>
                <button
              ...
              <form onSubmit={formik.handleSubmit} className="space-y-4 text-xs">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-neutral-450">Service Name</label>
                    <input
                      type="text"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-neutral-450">Delivery Days</label>
                    <input
                      type="number"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-neutral-450">Revision Limit</label>
                    <input
                      type="number"
                      {...formik.getFieldProps('revision_limit')}
                      className="h-9 px-3 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 outline-none focus:border-indigo-500"
                    />
                  </div>
                </div>
