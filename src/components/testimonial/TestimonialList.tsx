...
import { Search, Filter, Star, Download, Plus, Pencil } from 'lucide-react';
...
export function TestimonialList({
  ...
  const columns: ColumnDef<Testimonial>[] = [
      ...
      cell: (row) => (
        <div className="flex text-amber-500">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} className={`h-3.5 w-3.5 ${i < row.rating ? 'fill-current' : 'opacity-25'}`} />
          ))}
        </div>
      ),
    },
    {
      header: 'Quote',
      cell: (row) => (
        <div className="text-xs text-neutral-500 dark:text-neutral-400 line-clamp-2 italic max-w-sm">
          “{row.quote}”
        </div>
      ),
    },
      ...
      cell: (row) => (
        <div className="flex flex-wrap gap-1">
          {row.published ? (
            ...
            <span className="inline-flex items-center rounded-md border border-border bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 text-xs text-neutral-500 font-medium">
              Draft
            </span>
          )}
          {row.featured && (
      ...
      <div className="shrink-0 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onExport}
              className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              <Download className="h-4 w-4" /> Export CSV
            </button>
            <button
              onClick={onAdd}
              className="inline-flex items-center gap-2 rounded-md bg-brand-gradient px-4 py-2 text-sm font-medium text-white shadow-lg shadow-brand/20 transition-opacity hover:opacity-90"
            >
